...
function verifyAcres() {
    ...
    try {
        if (!(acresBox.value > 0)) {
            throw "please enter a number of acrres greater then 0.";
        }
    } catch (message) {
        validity = false;
        messageText = message;
        acresBox.value = ""; //removed errnoneous entry input box
    } finally {
...
function verifyMonths() {
    ...
    try {
        ...
        validity = false;
        messageText = message;
        monthsBox.value = ""; //removed errnoneous entry input box
    } finally {
...
function createRecommendation() {
    if (acresBox.value >= 5000) { // 5000 acres or less, no crop test needed
        ...
        } else { // 10+ months of farming per year // 10+ months of farming per year
